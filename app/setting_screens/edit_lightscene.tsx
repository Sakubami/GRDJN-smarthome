...
export default function EditScene() {
    ...
    return (
        <SafeAreaView style={{ flex:1, backgroundColor:"#e6e6e6"}}>
            <DefaultHeader headerTitle="Lichtszene Bearbeiten" createSeparation={false}/>
            {showToast && <Toast message={"Der Name kann nicht leer sein"}/>}
            {showToast1 && <Toast message={"Lichtszene gespeichert!"}/>}
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding: 16}}>
              <FancyTextBox onTextChange={setTileTitle} title={"Name"} placeholder={tile.title}/>
                <View style={{flexDirection: "row", justifyContent: "space-between", alignContent: "center", padding: 10}}>
                    <MenuButton icon={Save} onPress={handleSave}/>
                    <Text style={{color: "#000", margin: 3, fontWeight: 600}}>Lichtszene speichern</Text>
                    <Text style={{color: "#000", marginRight: 124}}></Text>
                </View>
            </ScrollView>
        </SafeAreaView>
    );
